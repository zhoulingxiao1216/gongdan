...
function runPCChecks(modelSource,appSource){
  ...
  let db=M.seed(),g=M.getGood(db,'DDEMO001');
  ...
  check('重复请求返回原记录',M.submit(db,d).duplicate&&db.records.length===6);
  ...
  function environment(){
    ...
    const injected=appSource.replace('render();\n})();',"render();window.__pcTest={state:()=>S,data:()=>db,openReport,reportSubmit,render,navigate,lookup,renderOrders,renderOrder,renderStaff,renderRecords};\n})();");
  ...
  let e=environment(),s=e.ready(3);e.app.reportSubmit();e.app.reportSubmit();e.flush();
  check('界面提交连点只写入一笔',e.app.data().records.length===6);
  check('提交后留在工作台并清空表单',s.page==='work'&&s.draft.qty===''&&!s.dirty&&!!s.lastSuccess);
  e.click('increase');check('模拟上游更新不计人员产出',e.app.data().records.length===6);
  s.draft.qty='2';s.draft.confirmed=true;s.dirty=true;e.app.reportSubmit();
  check('表单旧版本先要求复核',e.app.data().records.length===6);
  e.app.reportSubmit();e.flush();check('复核后增量可提交',M.done(e.app.data(),s.goods,s.task)===10);
  e=environment();s=e.ready(3);s.mode='fail';e.app.reportSubmit();e.flush();
  check('明确失败不写记录且保留草稿',e.app.data().records.length===5&&s.draft.qty==='3');
  e.app.reportSubmit();e.flush();check('失败重试成功一次',e.app.data().records.length===6);
  e=environment();s=e.ready(2);s.mode='unknown';e.app.reportSubmit();e.flush();
  check('响应丢失显示结果确认',s.dialog.type==='unknown'&&e.app.data().records.length===6);
  e.click('resolve-result');check('确认原结果不重复记账',!s.dialog&&e.app.data().records.length===6&&!s.dirty);
  ...
  e.click('nav',{page:'staff'});e.click('discard');check('确认放弃进入员工产量',s.page==='staff'&&s.draft===null&&e.app.data().records.length===5);
  e.click('drill',{date:M.day(),person:'V026',task:'贴纸'});check('汇总下钻带入人员项目日期',s.page==='records'&&s.rf.person==='V026'&&s.rf.task==='贴纸'&&s.rf.start===M.day());
  ...
  e.click('report-tab',{value:'report'});s.draft.qty='1';s.draft.confirmed=true;s.dirty=true;e.app.reportSubmit();e.flush();
  check('抽屉报工成功',e.app.data().records.length===6&&s.drawer==='report');
  ...
  for(const page of ['orders','order','work','staff','records']){e.app.navigate(page);check(page+'页面生成无异常',s.page===page)}
